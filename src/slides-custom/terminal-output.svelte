...
<Transition visible entry="scale-in" duration={0.6} delay={0.2}>
	<div
		class="overflow-hidden rounded-lg p-8 text-left font-mono text-xl leading-relaxed transition-all duration-700 {step >=
		1
			? 'max-h-48 max-w-4xl'
			: 'max-h-[900px]'}"
	...
	>
		<div
			class="overflow-hidden transition-all duration-700 ease-in-out {step >=
			1
				? 'max-h-0 opacity-0'
